...
namespace Sir.Document
{
    public class DocumentReader : IDisposable
    {
        ...
        {
            var valueStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.val", collectionId)), int.Parse(sessionFactory.Config.Get("value_stream_read_buffer_size")));
            var keyStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.key", collectionId)));
            var docStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.docs", collectionId)), int.Parse(sessionFactory.Config.Get("doc_map_stream_read_buffer_size")));
            var valueIndexStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.vix", collectionId)));
            var keyIndexStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.kix", collectionId)));
            var docIndexStream = sessionFactory.OpenReadStream(Path.Combine(sessionFactory.Dir, string.Format("{0}.dix", collectionId)));
